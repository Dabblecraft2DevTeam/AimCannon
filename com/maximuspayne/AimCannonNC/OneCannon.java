...
public class OneCannon{
    ...
    public OneCannon(Location inloc, Plugin inplugin) {
	delay = 1000;
    timeout = 0;
	charged = 0;
	range = 10;
	...
	if (timeout + AimCannonConfig.timeout < new Date().getTime()) {
	    return true;
	} else {
	    return false;
	}
    ...
    public void fire3(final Player p) {
	  ...
	  //  @Override
	    public void run()
	    {
		    ...
		    fireShell(look.multiply((float)(AimCannonConfig.speed*charged)), p);
	    }
	    });
    }
	...
	new Thread() {
		...
		try {
		   ...
		   // sleep(500);
		    ...
		    fireUpdate(look.multiply((float)(AimCannonConfig.speed*charged)), p);
		    //shellThread(1);
    ...
    public boolean Charge(Player p, boolean leftClick) 
    {
    	...
    	if( inventory.getItem(4) == null || inventory.getItem(4).getTypeId() != 388 )
    	{
		    ...
		    //p.sendMessage("test1.");
			if( p.getWorld().getName().equalsIgnoreCase("WarWorld1") || p.getWorld().getName().equalsIgnoreCase("WarWorld3") )
			{
				//p.sendMessage("test2.");
...
;
  ...
  /*  public void fireTorpedo(final Player p, final Block b, final BlockFace torpHeading, final int torpDepth, final int delayShoot, final BlockFace spreadFace, final int spread){
    ...
    public boolean checkProtectedRegion(Player player, Location loc)
    {
    	if( wgp != null )
    	{
    		if( !loc.getWorld().getName().equalsIgnoreCase("warworld2") && !loc.getWorld().getName().equalsIgnoreCase("warworld1") && !loc.getWorld().getName().equalsIgnoreCase("warworld3") )
    		{
    			return true;
    		}
	    ...
	    public void run()
	    {
				...
				if( i >= dcDepth+freeFallHt || ( currentBlock.getRelative(BlockFace.DOWN,2).getTypeId() != 8 && currentBlock.getRelative(BlockFace.DOWN,2).getTypeId() != 9 && currentBlock.getRelative(BlockFace.DOWN,2).getTypeId() != 0 && currentBlock.getRelative(BlockFace.DOWN,2).getTypeId() != 79) )
				{
					...
					*/
					...
					NavyCraft.explosion(8,  currentBlock);
					mc = (NavyCraft) plugin.getServer().getPluginManager().getPlugin("NavyCraft");
					if( mc != null )
					{
